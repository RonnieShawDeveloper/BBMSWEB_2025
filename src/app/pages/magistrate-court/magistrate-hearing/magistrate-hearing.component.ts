...
@Component({
  selector: 'app-magistrate-hearing',
  templateUrl: './magistrate-hearing.component.html',
  styleUrls: ['./magistrate-hearing.component.scss']
})
export class MagistrateHearingComponent {

  @Input() selectedOffender: Offender = {} as Offender;
  @Output() onExit = new EventEmitter<boolean>();

  hearing: Hearings = {} as Hearings;
  ...
  ngOnInit(): void {
    // Get the hearing for this defendant by defendant id

    this.af.collection('users', ref => ref.where('id', '==', this.selectedOffender.id)).valueChanges().subscribe(res => {
