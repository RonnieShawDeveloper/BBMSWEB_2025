...
export class JusticeGuard  {
  canActivate(
    route: ActivatedRouteSnapshot,
    state: RouterStateSnapshot): Observable<boolean | UrlTree> | Promise<boolean | UrlTree> | boolean | UrlTree {
    // Check the member in local storage for the authJudge or authJudgeClerk property to be true
    if(localStorage.getItem('member')) {
      if(JSON.parse(localStorage.getItem('member')).authJudge || JSON.parse(localStorage.getItem('member')).authJudgeClerk || JSON.parse(localStorage.getItem('member')).authAdmin) {
